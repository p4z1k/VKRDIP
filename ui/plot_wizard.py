...
class PlotWizard(QDialog):
    ...
    def toggle_rental_fields(self, plot_type):
        ...
        for i in range(self.basic_layout.rowCount()):
            label = self.basic_layout.itemAt(i, QFormLayout.LabelRole)
            if label and label.widget().text() == "Срок аренды до:":
                field = self.basic_layout.itemAt(i, QFormLayout.FieldRole).widget()
                label.widget().setVisible(is_rental)
                field.setVisible(is_rental)
                break
    ...
    def calculate_area(self, coordinates):
        ...
        for i in range(len(coordinates)):
            ...
            lat2, lon2 = coords_rad[(i + 1) % len(coordinates)]
    ...
    def accept(self):
        ...
        if not self.coordinates:
            QMessageBox.warning(self, "Ошибка", "Задайте границы участка")
            return
        if self.plot_manager and not self.plot_id:
            try:
                plot_data = self.get_data()
                self.plot_manager.add_plot(
                    name=plot_data["name"],
